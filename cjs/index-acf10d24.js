...
const index = require('./index-2b16071a.js');
...
require('./index.es-34078dca.js');
require('./index-6720146a.js');
...
class CompositeBlock {
    ...
    toTree(nodeSet, end = this.end) {
        ...
        let tree = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(nodeSet.types[this.type], this.children, this.positions, end - this.from).balance(2048);
...
const DefaultBlockParsers = {
    ...
    IndentedCode(cx, line) {
        ...
        if (nest)
            cx.startNested(from, nest.startParse(cx.input.clip(end), from, cx.parseContext), tree => new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(cx.parser.nodeSet.types[Type.CodeBlock], [tree], [0], end - from));
        else
    ...
    HTMLBlock(cx, line) {
        ...
        if (!marks.length && nodeType == Type.HTMLBlock && cx.parser.htmlParser) {
            cx.startNested(from, cx.parser.htmlParser.startParse(cx.input.clip(to), from, cx.parseContext), tree => new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(cx.parser.nodeSet.types[nodeType], [tree], [0], to - from));
        }
        else {
...
class BlockContext {
    ...
    addNode(block, from, to) {
        if (typeof block == "number")
            block = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(this.parser.nodeSet.types[block], none, none, (to !== null && to !== void 0 ? to : this.prevLineEnd()) - from);
    ...
    forceFinish() {
        ...
        if (this.nested) {
            ...
            if (result.length > len)
                result = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(result.type, result.children.filter((_, i) => result.positions[i] <= len), result.positions.filter(p => p <= len), len);
    ...
    configure(spec) {
        ...
        if (nonEmpty(config.defineNodes)) {
            ...
            for (let s of config.defineNodes) {
                ...
                nodeTypes.push(stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeType.define({
                    id,
                    name,
                    props: group && [[stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeProp.group, group]]
                }));
            }
            nodeSet = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeSet(nodeTypes);
        }
...
let nodeTypes = [stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeType.none];
for (let i = 1, name; name = Type[i]; i++) {
    nodeTypes[i] = stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeType.define({
        id: i,
        name,
        props: i >= Type.Escape ? [] : [[stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeProp.group, i in DefaultSkipMarkup ? ["Block", "BlockContext"] : ["Block", "LeafBlock"]]]
    });
}
...
class Buffer {
    ...
    finish(type, length) {
        return stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree.build({
            buffer: this.content,
...
const DefaultInline = {
    ...
    HTMLTag(cx, next, start) {
        ...
        if (cx.parser.htmlParser) {
            let p = cx.parser.htmlParser.startParse(stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.stringInput(cx.slice(start, start + 1 + m[0].length)), 0, {}), tree;
...
function stampContext(nodes, hash) {
    for (let n of nodes) {
        ...
        if (n instanceof stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree && n.type.isAnonymous)
...
const parser = new MarkdownParser(new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeSet(nodeTypes), null, null, Object.keys(DefaultBlockParsers).map(n => DefaultBlockParsers[n]), Object.keys(DefaultBlockParsers).map(n => DefaultLeafBlocks[n]), Object.keys(DefaultBlockParsers), DefaultEndLeaf, DefaultSkipMarkup, Object.keys(DefaultInline).map(n => DefaultInline[n]), Object.keys(DefaultInline));
...
const data = /*@__PURE__*/stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.defineLanguageFacet({ block: { open: "<!--", close: "-->" } });
const commonmark = /*@__PURE__*/parser.configure({
    props: [
        /*@__PURE__*/stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.styleTags({
            "Blockquote/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.quote,
            HorizontalRule: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.contentSeparator,
            "ATXHeading1/... SetextHeading1/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading1,
            "ATXHeading2/... SetextHeading2/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading2,
            "ATXHeading3/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading3,
            "ATXHeading4/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading4,
            "ATXHeading5/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading5,
            "ATXHeading6/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading6,
            "Comment CommentBlock": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.comment,
            Escape: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.escape,
            Entity: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.character,
            "Emphasis/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.emphasis,
            "StrongEmphasis/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.strong,
            "Link/... Image/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.link,
            "OrderedList/... BulletList/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.list,
            "BlockQuote/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.quote,
            "InlineCode/... CodeBlock FencedCode": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.monospace,
            URL: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.url,
            "HeaderMark HardBreak QuoteMark ListMark LinkMark EmphasisMark CodeMark": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.processingInstruction,
            "CodeInfo LinkLabel": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.labelName,
            LinkTitle: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.string,
            Paragraph: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.content
        }),
        /*@__PURE__*/stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.foldNodeProp.add(type => {
        ...
        /*@__PURE__*/stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.languageDataProp.add({
            Document: data
        })
    ],
    htmlParser: /*@__PURE__*/index.htmlLanguage.parser.configure({ dialect: "noMatch" }),
});
/**
...
const extended = /*@__PURE__*/commonmark.configure([GFM, Subscript, Superscript, Emoji, {
        props: [
            /*@__PURE__*/stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.styleTags({
                "TableDelimiter SubscriptMark SuperscriptMark StrikethroughMark": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.processingInstruction,
                "TableHeader/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.heading,
                "Strikethrough/...": stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.strikethrough,
                TaskMarker: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.atom,
                Task: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.list,
                Emoji: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.character,
                "Subscript Superscript": /*@__PURE__*/stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.special(stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.content),
                TableCell: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.tags.content
            })
        ]
    }]);
/**
...
function mkLang(parser) {
    return new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Language(data, parser, parser.nodeSet.types.find(t => t.name == "Document"));
}
...
function addCodeLanguages(languages, defaultLanguage) {
    return {
        codeParser(info) {
            let found = info && stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.LanguageDescription.matchLanguageName(languages, info, true);
            if (!found)
                return defaultLanguage ? defaultLanguage.parser : null;
            if (found.support)
                return found.support.language.parser;
            return stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.EditorParseContext.getSkippingParser(found.load());
        }
    };
}
...
const insertNewlineContinueMarkup = ({ state, dispatch }) => {
    let tree = stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.syntaxTree(state);
    let dont = null, changes = state.changeByRange(range => {
        if (range.empty && markdownLanguage.isActiveAt(state, range.from)) {
            ...
            if (markup.length) {
                ...
                if (inner.node.name == "ListItem") {
                    ...
                    if (emptyLine && line.from > 0 && !/[^\s>]/.test(state.doc.lineAt(line.from - 1).text))
                        return { range: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.EditorSelection.cursor(from), changes: { from, to: range.from } };
                    // First item
            ...
            if (range.from - line.from < insert.length)
                insert = "";
            changes.push({ from, to: range.from, insert: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Text.of(["", insert]) });
            return { range: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.EditorSelection.cursor(from + 1 + insert.length), changes };
        }
        return dont = { range };
    });
    if (dont)
        ...
        if (range.empty && markdownLanguage.isActiveAt(state, range.from)) {
            ...
            if (markup.length) {
                ...
                if (range.from > innerEnd + line.from && !/\S/.test(line.text.slice(innerEnd, range.from - line.from)))
                    return { range: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.EditorSelection.cursor(innerEnd + line.from),
                ...
                if (range.from - line.from == innerEnd) {
                    ...
                    if (start < range.from)
                        return { range: stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.EditorSelection.cursor(start), changes: { from: start, to: range.from } };
                }
            }
        }
        return dont = { range };
...
function markdown(config = {}) {
    ...
    return new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.LanguageSupport(mkLang(parser.configure(extensions)), addKeymap ? stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Prec.extend(stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.keymap.of(markdownKeymap)) : []);
}
