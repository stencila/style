...
const _function = require('./function-a5c185a1.js');
const toastController = require('./toastController-b34130ac.js');
 ...
 */
function swap$1(ma) {
    return isLeft(ma) ? right$1(ma.left) : left$1(ma.right);
}
// -------------------------------------------------------------------------------------
// non-pipeables
// -------------------------------------------------------------------------------------
...
if (typeof WebSocket !== 'undefined') {
  ...
  ws = _commonjsHelpers.commonjsGlobal.WebSocket || _commonjsHelpers.commonjsGlobal.MozWebSocket;
...
function lookup$1(i, as) {
    return as === undefined ? function (as) { return lookup$1(i, as); } : isOutOfBound(i, as) ? none : some(as[i]);
}
...
const fetchJobDatum = (jobUrl) => jobUrl === undefined
  ...
  : _function._function.pipe(jobUrl, async (job) => {
...
const jobStatusString = (jobObject) => _function._function.pipe(jobObject, fold(() => 'Requesting session', () => 'Requesting session', jobErrorMessage, (job) => job.statusMessage, jobErrorMessage, (job) => job.statusMessage));
const SessionStatus = ({ session, job }, children) => (index.h("span", { class: {
    executableDocumentStatus: true,
    danger: isFailure(session),
    success: isSuccess(session),
  } }, _function._function.pipe(session, fold(() => '', () => (index.h("span", null,
  index.h("stencila-icon", { icon: "loader-2" }),
  index.h("span", null, jobStatusString(job)))), (refreshingErroredJob) => (index.h("span", null,
  index.h("stencila-icon", { icon: "loader-2" }),
  index.h("span", null, jobErrorMessage(refreshingErroredJob)))), (refreshingSession) => (index.h("span", null, isEmpty(children) ? ([
  index.h("stencila-icon", { icon: "loader-2" }),
  refreshingSession.status,
]) : (index.h("span", null, children)))), (err) => (index.h("span", null,
  index.h("stencila-icon", { icon: "error-warning" }),
  index.h("span", null, jobErrorMessage(err)))), () => (index.h("stencila-tooltip", { text: "Compute session is active" },
  index.h("stencila-icon", { icon: "pulse" }),
  isEmpty(children) ? null : index.h("span", null, children)))))));
...
const notify = toastController.toastController$1({
  position: toastController.ToastPositions.topCenter,
});
const errorNotification = () => notify.present('Couldn’t start a compute session', {
  type: toastController.ToastTypes.danger,
});
const StencilaExecutableDocumentToolbar = class {
  constructor(hostRef) {
    ...
    this.handleJobStatus = (job, onJobReady, onJobError) => _function._function.pipe(job, fold(() => {
    ...
    this.startExecutor = async (executorUrl) => {
      ...
      this.session = await sessionClient
        ...
        notify.present('Ready to run document', {
          type: toastController.ToastTypes.success,
        });
    ...
    this.executeHandler = async (code) => {
      if (isPending(this.session)) {
        notify.present('Please wait until a compute session is found', {
          type: toastController.ToastTypes.warn,
        });
        return code;
      }
    ...
    this.goToActiveNode = () => {
      var _a;
      _function._function.pipe(this.codeNodeSelector(), lookup((_a = this.activeNodeIndex) !== null && _a !== void 0 ? _a : -1), map$1((el) => {
        var _a;
    ...
    this.executionProgress = () => (index.h("stencila-tooltip", { text: "Jump to location", onClick: this.goToActiveNode }, index.h("stencila-icon", { icon: "focus-3" }), index.h("span", null, this.statusMessage)));
  }
  componentWillLoad() {
  ...
  render() {
    return (index.h(index.Host, { position: this.position }, index.h("stencila-toolbar", null, index.h("span", { slot: "start" }, index.h("stencila-button", { color: "stock", icon: "play", size: "small", onClick: this.runAll, disabled: this.codeCount <= 0, tooltip: this.codeCount <= 0
      ...
      ? 'Running'
      : ['Run', index.h("span", { class: "hidden-sm" }, " Document")])), index.h("span", { slot: "middle" }, index.h(SessionStatus, { job: this.job, session: this.session }, isRefresh(this.session) && this.executionProgress())), this.sourceUrl !== undefined && (index.h("span", { slot: "end" }, index.h("stencila-button", { color: "stock", href: this.sourceUrl, target: "_blank", rel: "nofollow noopener", icon: "external-link", size: "small", dataEl: "Toolbar/Project Source" }, "Source"))))));
  }
};
...
exports.stencila_executable_document_toolbar = StencilaExecutableDocumentToolbar;
