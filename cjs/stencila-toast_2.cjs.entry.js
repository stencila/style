...
const slotSelectors = require('./slotSelectors-20d2cae5.js');
...
const StencilaToast = class {
  constructor(hostRef) {
    ...
    this.styleActionButtons = () => {
      var _a, _b;
      const buttons = Array.from((_b = (_a = slotSelectors.getSlotByName(this.el)('actions')) === null || _a === void 0 ? void 0 : _a.children) !== null && _b !== void 0 ? _b : []).filter((el) => el.tagName.toLowerCase() === 'stencila-button' &&
  ...
  render() {
    return (index.h(index.Host, { type: this.type, position: this.position, dismissable: this.dismissable, onMouseEnter: this.pauseAutoDismiss, onMouseLeave: this.autoDismiss }, index.h("div", { class: "toastAccent" }, index.h("stencila-icon", { icon: this.getIconByType(), iconStyle: "fill" })), index.h("div", { class: "content" }, index.h("slot", { name: "title" }), index.h("slot", null), index.h("slot", { name: "actions" })), (this.dismissable || this.duration === 0) && (index.h("stencila-button", { color: "neutral", iconOnly: true, icon: "close", minimal: true, size: "small", onClick: this.dismiss, class: "closeButton" }))));
  }
