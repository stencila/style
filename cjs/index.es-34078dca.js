...
class FragmentCursor {
    ...
    nodeAt(pos) {
        ...
        for (;;) {
            ...
            if (next instanceof stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.TreeBuffer) {
                this.index[last]++;
...
class Parse {
    ...
    advanceStack(stack, stacks, split) {
        ...
        if (this.fragments) {
            ...
            for (let cached = this.fragments.nodeAt(start); cached;) {
                ...
                if (!(cached instanceof stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree) || cached.children.length == 0 || cached.positions[0] > 0)
                    break;
                let inner = cached.children[0];
                if (inner instanceof stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree)
                    cached = inner;
                else
                    break;
            }
    ...
    stackToTree(stack, pos = stack.pos) {
        if (this.parser.context)
            stack.emitContext();
        return stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree.build({ buffer: StackBufferCursor.create(stack),
    ...
    finishNested(stack, tree) {
        if (this.nestWrap)
            tree = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(this.nestWrap, tree ? [tree] : [], tree ? [0] : [], this.nestEnd - stack.pos);
        else if (!tree)
            tree = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.Tree(stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeType.none, [], [], this.nestEnd - stack.pos);
        let info = this.parser.findNested(stack.state);
...
class Parser {
    ...
    constructor(spec) {
        /// @internal
        this.bufferLength = stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.DefaultBufferLength;
        /// @internal
        this.strict = false;
        ...
        this.nodeSet = new stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeSet(nodeNames.map((name, i) => stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.NodeType.define({
    ...
    parse(input, startPos = 0, context = {}) {
        if (typeof input == "string")
            input = stencilaActionMenu_stencilaEditor_stencilaNodeList_entry.stringInput(input);
