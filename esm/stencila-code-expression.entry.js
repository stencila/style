import { h, r as registerInstance, H as Host, a as getElement } from './index-e3052a43.js';
import { H as He, X as Xe, J as Je, c } from './schema-d66ff4ac.js';
...
const renderNode = (node) => {
  ...
  else if (Xe('CodeTypes', node)) {
    return (h("pre", null,
      h("output", null, node.text)));
  }
  else if (Je('ImageObject', node)) {
    return h("stencila-image-object", { image: node });
  }
  else if (Je('Datatable', node)) {
    return h("stencila-data-table", { table: node });
  }
  return h("span", null, JSON.stringify(node));
};
const NodeRenderer = ({ node }) => (h("span", { class: "output" }, node !== undefined ? renderNode(node) : null));
...
const CodeExpressionComponent = class {
  constructor(hostRef) {
    ...
    this.dividerArrow = () => (h("svg", { class: "divider", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 8 24", preserveAspectRatio: "xMinYMin" }, h("path", { d: "M8 12L1 0H0v24h1l7-12z" })));
    ...
    this.generateContent = () => {
      return [
        h("span", { class: "actions" }, h("stencila-button", { "aria-label": "Run Code", class: "run", onClick: this.executeRef, color: "key", disabled: !this.executeHandler, isLoading: this.executeCodeState === 'PENDING', icon: "play", iconOnly: true, minimal: true, size: "xsmall", tooltip: "Run" }), h("stencila-button", { "aria-label": "Run Code", class: "sourceToggle", onClick: this.toggleCodeVisibility, color: "key", icon: this.isCodeVisible ? 'eye-off' : 'eye', iconOnly: true, minimal: true, size: "xsmall", tooltip: `${this.isCodeVisible ? 'Hide' : 'Show'} Code` }), h("span", { class: "text", contentEditable: true, onBlur: this.removeHoverState, tabIndex: this.isCodeVisible ? 0 : -1 }, h("slot", { name: slots.text }))),
        this.dividerArrow(),
        h("span", { class: { hidden: true, slot: true } }, h("slot", { name: "output" })),
...
export { CodeExpressionComponent as stencila_code_expression };
